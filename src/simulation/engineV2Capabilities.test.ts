...
describe('Absorption curve', () => {
  it('Draft 1 bands, continuous', () => {
    const f = calculateAbsorptionFactor;
    expect(f(0)).toBe(1);
    expect(f(0.5)).toBe(1);
    expect(f(0.7)).toBeCloseTo(1, 12);
    expect(f(0.85)).toBeCloseTo(0.95, 12);
    expect(f(1.0)).toBeCloseTo(0.9, 12);
    expect(f(1.125)).toBeCloseTo(0.825, 12);
    expect(f(1.25)).toBeCloseTo(0.75, 12);
    expect(f(1.375)).toBeCloseTo(0.65, 12);
    expect(f(1.5)).toBeCloseTo(0.55, 12);
    expect(f(1.6)).toBeCloseTo(0.47, 12);
    expect(f(1.6875)).toBeCloseTo(0.4, 12);
    expect(f(3)).toBe(0.4);
    // no cliff at any boundary
    for (const x of [0.7, 1.0, 1.25, 1.5, 1.6875]) {
      expect(Math.abs(f(x - 1e-9) - f(x + 1e-9))).toBeLessThan(1e-6);
    }
    // monotone non-increasing
    for (let x = 0; x < 2.5; x += 0.01) expect(f(x + 0.01)).toBeLessThanOrEqual(f(x) + 1e-12);
  });
});
...
describe('4. Transformation Load aggregates across simultaneous initiatives', () => {
  it('Consumer $10M + AI $10M + People $10M → 11 + 13 + 6 = 30', () => {
    ...
    expect(c.capability.transformationLoad).toBeCloseTo(30, 12);
    expect(c.capability.newCohorts.map(x => x.bucket)).toEqual(['consumer', 'aiProduct', 'people']);
    expect(c.capability.loadToCapacityRatio).toBeCloseTo(0.5, 12);
  });

  it('spreading $6M over five buckets → 7 + 6 + 8.2 + 3.6 + 4.8 = 29.6', () => {
    const c = calculateV2QuarterConsequence(getV2Baseline(), {
      quarter: 1, allocation: alloc({ consumer: 6, enterprise: 6, aiProduct: 6, people: 6, universityCredentials: 6 }), strategicEnvelope: 30,
    });
    expect(c.capability.transformationLoad).toBeCloseTo(29.6, 12);
  });
});

describe('5. Load below capacity does not penalize', () => {
  it('balanced $5M×5: load 25 of 60 (41.7%) → factor 1, effective = nominal', () => {
    const c = calculateV2QuarterConsequence(getV2Baseline(), {
      quarter: 1, allocation: alloc({ consumer: 5, enterprise: 5, aiProduct: 5, people: 5, universityCredentials: 5, cashReserve: 5 }), strategicEnvelope: 30,
    });
    expect(c.capability.transformationLoad).toBe(25);
    expect(c.capability.absorptionFactor).toBe(1);
...
describe('6. Over-capacity load reduces new capability creation', () => {
  it('$30M C/E/AI (synthetic $90M envelope): load 86 / 60 = 143.3% → factor 0.60333', () => {
    const c = calculateV2QuarterConsequence(getV2Baseline(), {
      quarter: 1, allocation: alloc({ consumer: 30, enterprise: 30, aiProduct: 30 }), strategicEnvelope: 90,
    });
    expect(c.capability.transformationLoad).toBe(86);
    expect(c.capability.absorptionFactor).toBeCloseTo(0.75 - (86 / 60 - 1.25) / 0.25 * 0.2, 12);
    expect(c.capability.absorptionFactor).toBeCloseTo(0.603333, 5);
    const consumer = target(c, 'consumer');
    expect(consumer.nominalNew).toBe(17);
    expect(consumer.effectiveNew).toBeCloseTo(10.256667, 5);
    expect(consumer.absorptionLoss).toBeCloseTo(6.743333, 5);
    expect(consumer.maturedThisQuarter).toBeCloseTo(5.128333, 5); // 50% of effective
    expect(target(c, 'ai').effectiveNew).toBeCloseTo(27 * 0.603333, 4);
...
describe('9–10. Cap at 100 and saturation waste is observable', () => {
  ...
  it('Consumer at 98 + $30M: realized +2, wasted 6.5 then 5.95 then 2.55; nominal 17 retained', () => {
    const b = getV2Baseline();
    ...
    expect(t1.effectiveNew).toBe(17);
    expect(t1.maturedThisQuarter).toBe(8.5);
    expect(t1.realized).toBe(2);
    expect(t1.wastedSaturation).toBe(6.5);
    expect(t1.closing).toBe(100);
    expect(target(out[1].c, 'consumer').wastedSaturation).toBeCloseTo(5.95, 12);
    expect(target(out[2].c, 'consumer').wastedSaturation).toBeCloseTo(2.55, 12);
    const totalWaste = out.reduce((s, o) => s + target(o.c, 'consumer').wastedSaturation, 0);
    expect(totalWaste).toBeCloseTo(15, 12);
    expect(out[0].c.capabilityFlags).toContain('CAPABILITY_SATURATION_WASTE');
    // Excess is not converted into anything else
    ...
    expect(out[0].state.cash).toBe(60); // ledger unaffected: 60 + 30 − 30
  });
});
...
describe('11. People investment raises Organizational Capacity on its schedule', () => {
  ...
  it('matured capacity feeds the next quarter’s absorption (People $30M, then overload: 86 / 66 → 0.70758)', () => {
    const { out } = runQuarters(getV2Baseline(), [
    ...
    expect(out[1].c.capability.absorptionFactor).toBeCloseTo(0.75 - ((86 / 66 - 1.25) / 0.25) * 0.2, 12);
    expect(out[1].c.capability.absorptionFactor).toBeCloseTo(0.707576, 5);
  });
});

describe('12. Same strategy, different Organizational Capacity → different effective gains under heavy load', () => {
  it('capacity 60 vs 100 with load 86', () => {
    const heavy = { quarter: 1, allocation: alloc({ consumer: 30, enterprise: 30, aiProduct: 30 }), strategicEnvelope: 90 };
    const low = calculateV2QuarterConsequence(getV2Baseline(), heavy);
    const high = calculateV2QuarterConsequence({ ...getV2Baseline(), organizationalCapacity: 100 }, heavy);
    expect(low.capability.absorptionFactor).toBeCloseTo(0.603333, 5);
    expect(high.capability.absorptionFactor).toBeCloseTo(1 - (0.86 - 0.7) / 0.3 * 0.1, 12); // 0.946667
    expect(target(low, 'consumer').effectiveNew).toBeCloseTo(10.256667, 5);
    expect(target(high, 'consumer').effectiveNew).toBeCloseTo(16.093333, 5);
    expect(target(high, 'consumer').effectiveNew).toBeGreaterThan(target(low, 'consumer').effectiveNew);
    // financial consequence identical — capability and ledger are independent
