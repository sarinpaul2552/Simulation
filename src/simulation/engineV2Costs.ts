...
export type V2CommitmentSource = keyof typeof V2_COST_CALIBRATION.commitments;
...
export function calculateV2CostConsequence(
  ...
  const fixedTarget = opening.fixedFloor + K.fixed.scaleStepPerRevenueAboveBaseline * Math.max(0, priorRevenue - K.fixed.baselineRevenue);
  const gap = fixedTarget - opening.fixedSemiFixed;
  const fixedAdjustment = gap * (gap >= 0 ? K.fixed.adjustUpSpeed : K.fixed.adjustDownSpeed);
  const fixedSemiFixed = Math.max(opening.fixedFloor, opening.fixedSemiFixed + fixedAdjustment);
  ...
  const consumerAcquisitionSpend = v.consumerAcquisitionSpend * Math.max(0, market.consumerDemand);
  ...
  const bySource: Record<V2CommitmentSource, number> = { people: 0, enterprise: 0, aiProduct: 0 };
  for (const c of active) bySource[c.source] += c.quarterlyCost;
  const commitmentsTotal = bySource.people + bySource.enterprise + bySource.aiProduct;

  const createdThisQuarter: V2CostCommitment[] = (Object.keys(K.commitments) as V2CommitmentSource[])
  ...
  return {
    ...
    fixedTarget,
    fixedAdjustment: fixedSemiFixed - opening.fixedSemiFixed,
    fixedSemiFixed,
    variable: {
      consumerServicing,
